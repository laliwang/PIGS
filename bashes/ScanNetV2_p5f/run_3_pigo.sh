...
pigo_ratio=$(jq -r '.pigo_ratio' "${CONFIG_JSON}")
pigo_iter=$(jq -r '.pigo_iter' "${CONFIG_JSON}")
pigo_noload=$(jq -r '.pigo_noload' "${CONFIG_JSON}")
...
PIGO_folder="${Code_path}/PIGO_module/output_${Data_type}/${scene}_${pigo_ratio}_${pigo_iter}_${pigo_noload}"
PIGS_folder="${Data_path}/${Data_type}/scans_pigs"
Log_folder="${Code_path}/logs/${Data_type}/${scene}_${pigo_ratio}_${pigo_iter}_${pigo_noload}"
...
"CUDA_VISIBLE_DEVICES=${cudaid} python 11_train_scannet_ablation_p5f.py \
    -s ${Data_folder} \
    -m ${PIGO_folder} \
    --seg_path ${Seg_folder}/hive_pigs/${scene}_step/mvsa_output \
    --iterations ${pigo_iter} \
    --init_ratio ${pigo_ratio} \
    --no_preload ${pigo_noload} \
    --mask_type fusion \
    --use_temp --increment"
...
python 15_ball_pivoting.py \
    ...
    --output_mesh_path ${PIGS_folder}/${scene}/${scene}_planar_mesh_pigs_${pigo_ratio}_${pigo_iter}_${pigo_noload}.ply"
