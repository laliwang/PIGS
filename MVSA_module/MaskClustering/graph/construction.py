...
def mask_graph_construction(args, scene_points, frame_list, dataset):
    ...
    boundary_points, point_in_mask_matrix, mask_point_clouds, point_frame_matrix, global_frame_mask_list, mask_normal_dict = build_point_in_mask_matrix(args, scene_points, frame_list, dataset)
    ...
    nodes = init_nodes(global_frame_mask_list, visible_frames, contained_masks, undersegment_mask_ids, mask_point_clouds, mask_normal_dict)
...
def build_point_in_mask_matrix(args, scene_points, frame_list, dataset):
    '''
        ...
        Returns:
            ...
            mask_normal_dict: a dict where each key is the mask id in a frame, and the value is the normal tensor in world coordinate. 
    '''
    ...
    for frame_cnt, frame_id in iterator:
        mask_dict, normal_dict, frame_point_cloud_ids = frame_backprojection(dataset, scene_points, frame_id)
        ...
        for mask_id, mask_point_cloud_ids in mask_dict.items():
            ...
            mask_normal_dict[f'{frame_id}_{mask_id}'] = normal_dict[mask_id] # 采用跟mask_point_clouds同样的存储方式存储mask normal
    ...
    return boundary_points, point_in_mask_matrix, mask_point_clouds, point_frame_matrix, global_frame_mask_list, mask_normal_dict

def init_nodes(global_frame_mask_list, mask_project_on_all_frames, contained_masks, undersegment_mask_ids, mask_point_clouds, mask_normal_dict):
    nodes = []
    for global_mask_id, (frame_id, mask_id) in enumerate(global_frame_mask_list):
        ...
        normal = mask_normal_dict[f'{frame_id}_{mask_id}']
        node_info = (0, len(nodes))
        node = Node(mask_list, frame, frame_mask, point_ids, node_info, None, normal)
        nodes.append(node)
    return nodes
...
def process_one_mask(point_in_mask_matrix, boundary_points, mask_point_cloud, frame_list, global_frame_mask_list, args):
    ...
    '''
    visible_frame = torch.zeros(len(frame_list)) # 每个mask的visible_frame都是帧长度的0-1张量
