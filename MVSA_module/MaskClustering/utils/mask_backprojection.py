...
def turn_mask_to_point(dataset, scene_points, mask_image, mask_normal, frame_id):
    ...
    mask_normal = torch.from_numpy(mask_normal).cuda().reshape(-1,3)
    ...
    for mask_id in ids:
        ...
        normal_id = mask_normal[segmentation].mean(axis=0)
        normal_id = normal_id / torch.norm(normal_id) # normalize for normal vector
        ...
        mask_normal_list.append(normal_id)
    ...
    for i, mask_id in enumerate(initial_valid_mask_ids):
        ...
        mask_normal_info[mask_id] = mask_normal_list[i]
        frame_point_ids.update(mask_info[mask_id])

    return mask_info, mask_normal_info, valid_mask_ids, list(frame_point_ids)


def frame_backprojection(dataset, scene_points, frame_id):
    mask_image, mask_normal = dataset.get_segmentation(frame_id, align_with_depth=True)
    mask_info, mask_normal_info, _, frame_point_ids = turn_mask_to_point(dataset, scene_points, mask_image, mask_normal, frame_id)
    return mask_info, mask_normal_info, frame_point_ids