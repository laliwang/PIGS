...
def loadClassMap(tsv_path):
    classMap = {}
    classLabelMap = {}
    with open(tsv_path) as info_file:
        line_index = 0
...
def process_scene(scene_id, scannet_folder, output_folder, tsv_path):
    if str(scannet_folder).split("/")[-4] == "ScanNet++":
        filename = scannet_folder / scene_id / (scene_id + ".aggregation_pp.json")
    else:
	...
	# "id": 0,
	# "objectId": 0,
	# "segments": [185829,142299,141829,141259,140281,185072,185576,141290,142257,138914,140550,140228],
	# "label": "office chair"
	# }
    ...
    #mesh的segmentation中没有和别的顶点划分为一个簇的就称为unannotated?
    numSegments = 0
    ...
    segmentEdges = []
    for faceIndex in trange(faces.shape[0]):
    ...
    classMap, classLabelMap = loadClassMap(tsv_path=tsv_path)
    ...
    debugIndex = -1
    pbar=tqdm(total=len(groupSegments))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tsv_path', type=str, required=True, help="Path to tsv file")
    parser.add_argument(
    ...
    for index, scene_id in enumerate(tqdm(scene_ids)):
        # if scene_id[:5] != "scene":
        #     continue
        if scene_id != args.scene_name:
            continue
        num_planes = process_scene(scene_id, scannet_folder=args.scannet, output_folder=args.output, tsv_path=args.tsv_path)
