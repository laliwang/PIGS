...
class ResultsAverager:
    ...
    def print_sheets_friendly(
        ...
        for k, v in metrics_to_print.items():
            ...
            # some slightly hacky code to format different results in different ways

            if k in ("precision↑", "recall↑", "f1_score↑"):
                # multiply by 100 (to make into a percentage) and print with 2dp
                metric_string = f"{v * 100:.2f},"

            elif k in ("compl↓", "acc↓", "chamfer↓"):
                # multiply by 100 (to make into cm) and print with 2dp
                metric_string = f"{v * 100:.2f},"

            elif k in ("ri↑", "sc↑"):
                # 3dp without leading zeros – might be a better way to do this?
                metric_string = f"{v:.3f},".lstrip("0")
            elif k in ("top_planes_compl↓", "top_planes_acc↓"):
                # multiply by 100 (to make into cm) and print with 2dp
                metric_string = f"{v * 100:.2f},"
            else:  # e.g. voi↓
                # 3dp with leading digit
                metric_string = f"{v:.3f},"
    ...
    def output_json(self, filepath, mesh_name, print_running_metrics=False):
        """
        ...
        for k, v in metrics_to_use.items():
            ...
            metrics_row += f"{metric_string:8} "
            scores_dict[mesh_name]["scores"][k] = float(v)

        scores_dict = OrderedDict(
            natsorted(scores_dict.items(), key=lambda x: x[0])
        )
