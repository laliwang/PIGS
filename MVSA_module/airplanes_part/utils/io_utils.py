...
from airplanes_part.utils.volume_utils import SimpleVolume
...
def load_gt_bundle(scene_path: Path, scene_name: str, load_visibility_volumes: bool = True):
    ...
    if not volume_path.exists():
        volume_path = Path(str(scene_path).replace('plane_meshes', 'visibility_volumes')) / AssetFileNames.get_visibility_volume_filename(scene_name)
    # print(f"Loading visibility volume from {volume_path}")
    ...
    return MeshingBundle(
        ...
        instances=instances,
        visibility_path=volume_path
    )


def load_pred_bundle(scene_path: Path, scene_name: str, mesh_name: str,):
    """
    Load a mesh from a file
    Params:
        scene_path: path to the scene directory
    ...
    planar_mesh_filepath = scene_path / (AssetFileNames.get_planar_mesh_filename(scene_name)).replace('.ply', f'_{mesh_name}.ply')
    print(f"Loaded and evaluating planar mesh of {planar_mesh_filepath}")
    planar_mesh = None
