...
from align import align_depth_scale
...
def project_mask_pc(rgb_input, depth_input, depth_color, pose_input, K_input, max_depth=10, filter_outlier=False, v_size=0.025, depth_scale=1000.0, crop=6, depth_folder=None):
    ...
    if depth_color is not None:
      ...
      cv2.imwrite(os.path.join(depth_folder, f'{frame_id}.png'), depth_16)
...
if __name__ == '__main__':
  #### prepare data
    ...
    parser.add_argument("--pre_align", action="store_true", default=False)
    ...
    # depth_m_folder = os.path.join(input_folder, f'depth_{depth_model}') if not args.pre_align else os.path.join(input_folder, f'depth_{depth_model}_align')
        ...
        with tqdm(total=len(rgb_list)) as pbar:
            for i in range(len(rgb_list)):
                ...
                if depth_file is not None:
                    ...
                    # 2026-1-24 depth align with gt_depth for replica
                    if args.pre_align:
                      ori_h, ori_w = pred_depth.shape
                      gt_depth_1 = gt_depth.reshape(-1)
                      pred_depth_1 = pred_depth.reshape(-1)
                      weight = ((pred_depth_1 - gt_depth_1) <= 0.5) & (gt_depth_1 > 0.01)
                      d_scale = align_depth_scale(pred_depth.reshape(1, -1), gt_depth.reshape(1, -1), weight.reshape(1, -1).float())
                      pred_depth = pred_depth * d_scale.item()
                      pred_depth = pred_depth.reshape(ori_h, ori_w)
                ...
                pred_pcd, depth_color_filter = project_mask_pc(rgb_file, pred_depth.cpu().numpy(), depth_color, pose_file, K_input, crop=crop, depth_folder=depth_m_folder)
