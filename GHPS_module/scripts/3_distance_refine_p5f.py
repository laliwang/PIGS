...
def show_anns_mobile(masks_sam, invalid, kernel_size=(9,9), open_flag=True, coarse_flag=False):
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, kernel_size)
    if masks_sam is None:
        return np.zeros((invalid.shape[0], invalid.shape[1], 3)), np.zeros((invalid.shape[0], invalid.shape[1]))
    sorted_masks = masks_sam[np.argsort(-masks_sam.sum(axis=(1,2)))].astype(np.bool_)  # (n_masks, h, w)
    n_masks = sorted_masks.shape[0]
    anns_color = np.zeros((sorted_masks[0].shape[0], sorted_masks[0].shape[1], 3))
    anns_npy = np.zeros((sorted_masks[0].shape[0], sorted_masks[0].shape[1]))
    idx = 1
...
def decompose_sam_by_normal(mask_n_list, mask_sam_input, min_size=1000, post_dict=None, dist_input=None):
    if len(np.unique(mask_sam_input)) == 1:
        return np.zeros((mask_sam_input.shape[0], mask_sam_input.shape[1], 3)), np.zeros_like(mask_sam_input)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--sam_type', type=str, default='sam.vit_h', help='sam variants')
    ...
    if 'mobile' in args.sam_type:
        # import mobilesam from local path
        sys.path.insert(0, '/code1/wjh/VLM_model/MobileSAM/MobileSAMv2')
        from x_mobile_sam import get_mobile_sam, mobile_predictor
        weight_folder = args.weight_pth
        predictor, sam_model, obj_model = get_mobile_sam(weight_folder)
        params = {'retina': True,
              'imgsz': 1024,
              'conf': 0.4,
              'iou': 0.9}
        print('mobile sam model loaded')
    elif 'fast' in args.sam_type:
        # import fastsam from local path
        sys.path.insert(0, '/code1/wjh/VLM_model/FastSAM')
        from x_fast_sam import get_fast_sam, fast_predictor
        weight_folder = args.weight_pth
        ...
        params = {'device': 'cuda' if torch.cuda.is_available() else 'cpu',
              ...
              'conf': 0.4,
              'iou': 0.9}
        print('fast sam model loaded')
    else:
        # import original sam from public pip package
        from segment_anything import SamAutomaticMaskGenerator, sam_model_registry
        pth_dict = {'vit_h': 'sam_vit_h_4b8939.pth', 'vit_l': 'sam_vit_l_0b3195.pth', 'vit_b': 'sam_vit_b_01ec64.pth'}
        model_scale = (args.sam_type).split('.')[-1]
        sam = sam_model_registry[model_scale](checkpoint=os.path.join(args.weight_pth, pth_dict[model_scale]))
        ...
        print(f'original sam model of {model_scale} loaded')

    # basic input and output folder paths
    ...
    # for idx in trange(10):
        ...
        with torch.cuda.amp.autocast():
            if 'mobile' in args.sam_type:
                masks_sam = mobile_predictor(predictor, sam_model, obj_model, img_ori if args.use_ori else img, params)
                _, mask_npy = show_anns_mobile(masks_sam, mask_invalid, open_flag=args.open_flag, coarse_flag=args.coarse_flag)
            elif 'fast' in args.sam_type:
                masks_sam = fast_predictor(sam_model, img_ori if args.use_ori else img, params)
                _, mask_npy = show_anns_mobile(masks_sam, mask_invalid, open_flag=args.open_flag, coarse_flag=args.coarse_flag)
            else:
                masks_sam = mask_generator_2.generate(img_ori if args.use_ori else img)
                _, mask_npy = show_anns(masks_sam, mask_invalid, open_flag=args.open_flag, coarse_flag=args.coarse_flag)
