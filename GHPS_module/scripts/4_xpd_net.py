...
sys.path.append(
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "X-PDNet")
    )
)
...
# to process overlapping xpd mask
def remove_small_masks(mask, min_area=50):
    """
    删除单个掩码中的小块（连通域面积小于 min_area）。
    :param mask: 单个二值掩码 (H, W)
    ...
    for i in range(1, num_labels):  # 忽略背景标签 0
        if stats[i, cv2.CC_STAT_AREA] >= min_area:
            mask_cleaned[labels == i] = 1  # 保留大于最小面积的连通域
    
    return mask_cleaned

def mask_filter(mask_input):
    """
    掩码过滤：
    1. 删除被完全覆盖的掩码，for hive-gaussian
    2. 删除相交区域
    """
    if mask_input.ndim < 3:
        return mask_input, None

    mask_sum = np.sum(mask_input, axis=0)  # 统计每个像素的覆盖次数
    mask_inter = mask_sum > 1  # 找到所有被多个掩码覆盖的区域

    # 筛选掩码：删除被其他掩码完全覆盖的掩码
    mask_list = []
    mask_s_list = []
    for mask in mask_input:
        mask = remove_small_masks(mask, min_area=1000)
        if np.sum(mask) == 0:  # 如果清理后掩码为空，则跳过
            continue
        inter = np.logical_and(mask, mask_inter)
        if np.sum(inter) / np.sum(mask) < 0.8:
            mask_list.append(mask)
        else:
            mask_s_list.append(mask)

    # 清理相交区域
    mask_sum = np.sum(mask_list, axis=0)
    mask_inter = mask_sum > 1
    mask_list = [np.where(mask_inter, 0, mask) for mask in mask_list]
    if (len(mask_s_list)>0):
        return np.stack(mask_list), np.stack(mask_s_list)
    else:
        return np.stack(mask_list), None

def mask_filter_small(mask_input):
    """
    掩码过滤：
    1. 保留被完全覆盖的掩码，删除覆盖它们的掩码的对应区域，for segmentation enhancement
    2. 删除相交区域。
    """
    if mask_input.ndim < 3: # dim小于3的说明是mask_npy中只有一个掩码的情况无需判断
        return mask_input
    ...
    # 清理小mask之间的相交区域
    def clear_inter(input_list):
        if len(input_list) == 0:
            return [] # 2025-08-05
        input_sum = np.sum(np.stack(input_list, axis=0),axis=0)
        input_inter = input_sum > 1
        output_list = [np.where(input_inter, 0, input) for input in input_list]
        return output_list
    
    # 清理大小mask自身之间的相交区域
    mask_list = clear_inter(mask_list)
    
    # 清理大mask上和小mask的相交区域
    mask_final_list = []
    if len(mask_s_list) > 0:
        mask_s_list = clear_inter(mask_s_list)
        # print('overlapped')
        for mask in mask_list:
            for ms in mask_s_list:
                # ms膨胀一下让掩码之间分隔的更远
                ms = cv2.dilate(ms.astype(np.uint8), np.ones((5, 5), np.uint8))
                mask = np.where(ms, 0, mask)
            mask_final_list.append(mask)
        mask_final_list = mask_final_list + mask_s_list
    else:
        mask_final_list = mask_list
    
    mask_final_list = [remove_small_masks(mask, min_area=1000) for mask in mask_final_list]
    
    return np.stack(mask_final_list)

def mask_gen(masks):
    mask_sum = np.sum(masks, axis=0)
    mask_inter = mask_sum > 1
    if mask_inter.sum() > 0:
        print(mask_inter.sum())
        print(masks.shape)
        assert False, "There are overlapping masks in the input image."
    mask_out = np.zeros_like(masks[0])
    for i in range(masks.shape[0]):
        mask_out[masks[i].astype(bool)] = i+1
    return mask_out

def mask_xpd(masks, crop, small_flag):
    if np.sum(masks > 0) == 0:
        return masks

    if small_flag:
        masks = mask_filter_small(masks)
    else:
        masks, masks_small = mask_filter(masks)
    if masks.ndim < 3:
        masks = np.expand_dims(masks, axis=0)

    mask_out = mask_gen(masks)
    if crop > 0:
        mask_out = mask_out[crop:-crop, crop:-crop].astype(np.uint8)
        mask_out = cv2.copyMakeBorder(mask_out, crop, crop, crop, crop, cv2.BORDER_CONSTANT, value=0).astype(np.float16)
    return mask_out

def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="XPDNet Inference")
    ...
    parser.add_argument("--config", default="XPDNet_50_config", help="The config object to use.")
    # Inference Settings
    ...
    # for solving the overlapping of xpd mask
    parser.add_argument('--crop', type=int, default=0, help='Crop the image for scannet v2 data.')
    parser.add_argument('--small', action="store_true", default=False, help='Whether to retain small masks contained by larger masks.')
    global args
...
def inference_image(net: XPDNet, path: str, save_path: str = None, depth_mode: str='colored', crop: int=0, small_flag: bool=False):
    ...
    if results[0]["pred_masks"] is not None:
        mask_np = results[0]["pred_masks"].squeeze(0).cpu().numpy()
        mask_np = mask_xpd(mask_np, crop=crop, small_flag=small_flag)
        np.save(mask_path, mask_np)
    else:
        mask_np = np.zeros((frame_np.shape[0], frame_np.shape[1]))
        np.save(mask_path, mask_np)
    return mask_np.shape
    # cv2.imwrite(save_path, blended_frame)
    
   
def inference_images(net: XPDNet, in_folder: str, out_folder: str, max_img: int=0, depth_mode: str='colored', crop: int=0, small_flag: bool=False):
    ...
    for p in sorted(input_list):
        ...
        xpd_shape = inference_image(net, img_path, out_path, depth_mode=depth_mode, crop=crop, small_flag=small_flag)
        print("Inference images: " + os.path.basename(img_path) + ' -> ' + os.path.basename(out_path),  end='\r')
        index = index + 1
        if index >= max_img:
            break
    print(f'XPD Planar mask saved at shape: {xpd_shape}')
    print("Done.")


def ibims1(net: XPDNet, in_folder: str, out_folder: str):
    if not os.path.exists(out_folder):
        os.mkdir(out_folder)
    print()
    index = 0
...
def ibims1_pd(net: XPDNet, in_folder: str, out_folder: str):
    if not os.path.exists(out_folder):
        os.mkdir(out_folder)
    print()
    index = 0
    ...
    for p in sorted(input_list):
        ...
        pred_depth[pred_depth>=10] = np.nan
...
if __name__ == "__main__":
    ...
    if args.images is not None:
        ...
        if not os.path.exists(out):
            os.makedirs(out)
        inference_images(net, inp, out, max_img=args.max_img, depth_mode=args.depth_mode, crop=args.crop, small_flag=args.small)
