...
def inference_image(net: XPDNet, path: str, save_path: str = None, depth_mode: str='colored'):
    ...
    # print(results[0]["pred_masks"].shape)
    ...
    if results[0]["pred_masks"] is not None:
        ...
        np.save(mask_path, np.zeros((frame_np.shape[0], frame_np.shape[1])))
    # cv2.imwrite(save_path, blended_frame)
    
    zx = cv2.Sobel(depth, cv2.CV_64F, 1, 0, ksize=7)
    zy = cv2.Sobel(depth, cv2.CV_64F, 0, 1, ksize=7)
...
def inference_images(net: XPDNet, in_folder: str, out_folder: str, max_img: int=0, depth_mode: str='colored'):
