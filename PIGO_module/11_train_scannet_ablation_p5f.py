...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, voxel_size, init_w_gaussian, args):
    ...
    hive_dict['init_ratio'] = args.init_ratio   # 是否对初始化点云进行降采样
...
if __name__ == "__main__":
    ...
    parser.add_argument('--init_ratio', type=float, default=0.0)
    parser.add_argument('--no_preload', type=bool, default=False)

    args = parser.parse_args(sys.argv[1:])
    args.save_iterations.append(args.iterations)

    args.preload_img = not args.no_preload
