...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
        ...
        if self.preload_img:
            # preload_img与否都无所谓其实，preload之后会难免地让显存占用显著增加吧
            image = Image.open(self.image_path)
            resized_image = image.resize((image_width, image_height))
            resized_image_rgb = PILtoTorch(resized_image) # 这么方便的函数直接PIL类型转torch类型呢
            if ncc_scale != 1.0:
            ...
            self.original_image = resized_image_rgb[:3, ...].clamp(0.0, 1.0).to(self.data_device) # Image库读取的图像数值默认介于0到1之间而非最大255呢
            ...
            if os.path.exists(mask_path):
                ...
                self.mask = (self.mask < 0.5).to(self.data_device) # DTU数据集应该是只考虑对前景的渲染所以可能需要mask吧
        ...
        self.trans = trans # trans是啥东东诶 
        self.scale = scale

        self.world_view_transform = torch.tensor(getWorld2View2(R, T, trans, scale)).transpose(0, 1).cuda() # R,T本来就是了 但是为了方便trans和scale写了个函数
        # projection_matrix 和 full_proj_transform 好像没怎么见过
        # projection_matrix 是将相机坐标系到标准坐标系的投影矩阵 full_proj_transform其实就是世界坐标系到标准坐标系的投影全过程
        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0,1).cuda()
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3] # camera_center应该还是世界坐标系下的坐标
        self.camera_rot = self.world_view_transform.inverse()[:3, :3]
        self.plane_mask, self.non_plane_mask = None, None

    def get_image(self):
        if self.preload_img:
            return self.original_image.cuda(), self.image_gray.cuda(), None
        else:
            image = Image.open(self.image_path)
            ...
            return gt_image.cuda(), gt_image_gray.cuda(), None

    def get_calib_matrix_nerf(self, scale=1.0):
        intrinsic_matrix = torch.tensor([[self.Fx/scale, 0, self.Cx/scale], [0, self.Fy/scale, self.Cy/scale], [0, 0, 1]]).float()
        extrinsic_matrix = self.world_view_transform.transpose(0,1).contiguous() # cam2world 为什么变换矩阵的转置就直接相当于逆了啊
        return intrinsic_matrix, extrinsic_matrix
    
    def get_rays(self, scale=1.0):
        # get_rays得到的rays_d一眼就是标准坐标系下
...
class DepthCamera(nn.Module):
    def __init__(self, colmap_id, R, T, mat, FoVx, FoVy,
                 image_width, image_height,
                 image_path, image_name, depth_path, depth_name, 
                 mask_xpd_path,mask_xpd_name,
                 max_depth, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, 
                 ncc_scale=1.0, depth_scale=1000.0,
                 preload_img=True, data_device = "cuda"
                 ):
        ...
        self.mask_xpd_name = mask_xpd_name
        self.mask_xpd_path =mask_xpd_path 
        ...
        self.original_image, self.image_gray, self.depth_image ,self.mask_image= None, None, None,None
        self.preload_img = preload_img
        self.ncc_scale = ncc_scale
        if self.preload_img:
            # preload_img与否都无所谓其实，preload之后会难免地让显存占用显著增加吧
            ...
            resized_image_gray = resized_image.convert('L')
            resized_image_gray = PILtoTorch(resized_image_gray)
            image_depth = Image.open(self.depth_path)
            resized_depth = image_depth.resize((image_width, image_height))
            resized_depth = (np.asarray(resized_depth, dtype=np.float32) / self.depth_scale)
            resized_depth = torch.from_numpy(resized_depth)
             
            self.original_image = resized_image_rgb[:3, ...].clamp(0.0, 1.0).to(self.data_device) # Image库读取的图像数值默认介于0到1之间而非最大255呢
            self.image_gray = resized_image_gray.clamp(0.0, 1.0).to(self.data_device)
            self.depth_image = resized_depth.to(self.data_device)
            ...
            if self.mask_xpd_path is not None:
                ...
                # resized_mask = cv2.resize(image_mask, (image_width, image_height), interpolation=cv2.INTER_NEAREST).astype(np.float32)
        ...
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3] # camera_center应该还是世界坐标系下的坐标
        self.camera_rot = (self.world_view_transform.inverse()[:3,:3]).T
        self.plane_mask, self.non_plane_mask = None, None

    def get_image(self):
        if self.preload_img:
            if self.mask_xpd_path is not None:
                return self.original_image.cuda(), self.image_gray.cuda(), self.depth_image.cuda(),self.mask_image.cuda()
            else:
                return self.original_image.cuda(), self.image_gray.cuda(), self.depth_image.cuda(), None
        else:
            image = Image.open(self.image_path)
            resized_image = image.resize((self.image_width, self.image_height))
            resized_image_rgb = PILtoTorch(resized_image)
            ...
            image_depth = Image.open(self.depth_path)
            resized_depth = image_depth.resize((self.image_width, self.image_height))
            resized_depth = (np.asarray(resized_depth, dtype=np.float32) / self.depth_scale)
            resized_depth = torch.from_numpy(resized_depth)
